          ...
          <div v-for="item in teams.data" class="mb-6 lg:mb-0">
            <div
              ...
              <div class="p-6">
                ...
                <p class="cursor-pointer mb-4 text-neutral-500 dark:text-neutral-300">{{item.attributes.position}}</p>
                <NuxtLink :to="`/teams/${item.id}`" class="px-2">
                  <!-- GitHub -->
                  Подробнее
                </NuxtLink>
              </div>
...
export default {
  ...
  async asyncData({$axios}) {
    let teams = await $axios.$get(constants.baseApiUrl + '/teams?populate=*')
    return {teams}
  },
  methods: {
